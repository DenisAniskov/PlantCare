...
class PlantCareViewModel(
    ...
    fun markEventDone(context: Context, event: CareEvent) {
        viewModelScope.launch {
            ...
            var nextReminder = event.reminderDateTime
            if (event.reminderEnabled && event.intervalDays != null) {
                val intervalMs = (event.intervalDays * 24f * 60f * 60f * 1000f).toLong()
                nextReminder = (event.reminderDateTime ?: now) + intervalMs
            }
            val updatedEvent = event.copy(done = true, lastDate = now, reminderDateTime = nextReminder)
